...
//удаление одного документа
    const DeleteOneDocs = (docsId,docsName) => {
        runInAction(() => {
            AdminRusadaStore.mediaDel.push(docsName)
            AdminRusadaStore.rusada.docs.splice(docsId,1)
        })
    };
    ...
    return (
        <div className={classes.item}>
            <TextField
                ...
                value={AdminRusadaStore.rusada.docs[props.index].title}
                onChange={(e) => {
                    runInAction(() => {
                        AdminRusadaStore.rusada.docs[props.index].title = (e.target.value)
                    })
                }}
                variant="outlined"
                multiline
                minRows={1}
                maxRows={10}
                disabled={!AdminRusadaStore.rusada.edit}
            />
...
export default observer(RusadaDocsItem);
